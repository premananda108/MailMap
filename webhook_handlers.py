...
import email_utils  # Direct import


# datetime import was removed as it's not used

# Проблемы в текущем коде:

# 1. Проверка лимита происходит только ПЕРЕД обработкой всех вложений
# 2. Если пользователь может загрузить еще несколько фото, но не все из письма,
#    то все фото отклоняются или все принимаются
# 3. Инкремент счетчика происходит ПОСЛЕ сохранения контента
                ...
                try:
                    ...
                    if current_image_url:
                        ...
                        if content_id:
                            ...
                            if user_id_for_content:
                                try:
                                    user_ref = db_client.collection('users').document(user_id_for_content)
                                    user_ref.update({
